...
def render_video(results_path, sig=2.0, asig=0.25, trim_first_frame=True):
	...
	#print(video_temporal_relevance.shape)
	#print(video_spatial_relevance.shape)
	#print(video_input.shape)


	# Concatenate individual outputs to be video level
	#audio_input = np.concatenate([audio for audio in audio_input[:][:5]], axis=1)
	#audio_temporal_relevance = np.concatenate([audio for audio in audio_temporal_relevance[:][:5]], axis=2)
	#audio_spectral_relevance = np.concatenate([audio for audio in audio_spectral_relevance[:][:5]], axis=2)

	video_input = [frame for clip in video_input[:][:5] for frame in clip[offset]]
	#print(len(video_input))
	#print(video_input[0].shape)
	#print(video_temporal_relevance[:, sig, ...].shape)
	...
	#print(len(video_temporal_relevance))
	#print(video_temporal_relevance[0].shape)
	...
	camera = Camera(fig)
	#     fig.suptitle(f"Ground truth: {results_path.split('/')[-3]}, Prediction: {prediction}", fontsize=12)

	#print(video_temporal_relevance[0].shape)
	#print(len(video_temporal_relevance))
	#print(len(video_input))
	#print(len(video_spatial_relevance))

	video_data = zip(video_input, video_temporal_relevance, video_spatial_relevance)
	#video_data_len = sum(1 for _ in video_data)
	#print(video_data_len)
	for idx, (input_frame, temporal_frame, spatial_frame) in enumerate(video_data):

		#print(input_frame.shape)
		#print(temporal_frame.shape)
		#print(spatial_frame.shape)

		input_frame = cv2.cvtColor(input_frame, cv2.COLOR_BGR2GRAY)
		#         plt.subplot(2, 3, 1)
		v_ax = fig.add_subplot(gs[0, 0])
		v_ax.imshow(input_frame, cmap='gray')
		v_ax.axis('off')
		...
		v_ax2.imshow(input_frame, cmap="gray")
		v_ax2.imshow(temporal_frame, alpha=0.5)
		v_ax2.axis('off')
		v_ax2.set_title("Temporal relevance")

		#         plt.subplot(2, 3, 3)
		v_ax3 = fig.add_subplot(gs[0,2])
		v_ax3.imshow(input_frame, cmap="gray")
		v_ax3.imshow(spatial_frame, alpha=0.5)
		v_ax3.axis('off')
		v_ax3.set_title("Spatial relevance")

		#         plt.subplot(2,3,4)
		a_ax = fig.add_subplot(gs[1,0])
		librosa.display.specshow(audio_input[idx // 16], **spectrogram_rendering_config)
		#plt.colorbar()
		...
		#print(relevance_spectrogram[asig].shape)
		librosa.display.specshow(
			relevance_spectrogram[idx // 16, asig, ...],
			cmap=temporal_cmap,
			**spectrogram_rendering_config,
			vmax=1)
	...
	animation = camera.animate(interval=62.5)
	animation.save(results_path + "/vadr_pred" + prediction + ".mp4")
#     return HTML(animation.to_html5_video())

	# NOTE:: commented out to save time when outputting to disk
#     with out:
#         out.clear_output()
...
# video_controls = interactive(render_video,
#                              results_path=results,
#                              sig=(0, 4, 0.25),
#                              asig=(0, 4, 0.25))

# print("Note: This is VERY slow to render (1 minute), so please be patient! ")
# VBox([video_controls, out])

if __name__ == "__main__":
	#results = glob.glob("./results/*/*/")
	#results = ["./results/Hammering/_DSC0507/"]
	#results = ["./results/Bowling/v_Bowling_g12_c03/"]
	#results = ["./results/CliffDiving/v_CliffDiving_g02_c03"]
	...
	results = ["./results5/CliffDiving/v_CliffDiving_g10_c06"]
	...
	results = ["./results5/PlayingDhol/v_PlayingDhol_g05_c03"]
